%Liebmanns_accelerated_method
function output_vector = Liebmanns_accelerated_method (grid_dimension,top_boundary_temperature,bottom_boundary_temperature,left_boundary_temperature,right_boundary_temperature,iteration_times,initial_guessing_temperature)
...
for o = 1:Z
    for x =N:-1:2
        for y =2:N
            ...
            V=updated_array;  %This statement ensures the next consecutive element reads the previous updated temperature.
        end
    end
    number_of_iteration=o
    V=updated_array
end
output=('Final Temperature Distribution using Liebmanns accelerated method')
V
x=[0:N];
y=(N:-1:0);
surf(x,y,V)
...
title('Temperature Distribution in the system using Liebmanns accelerated method')
end
